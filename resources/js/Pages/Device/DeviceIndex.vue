    ...
    <div class="ml-32 pt-20 pb-20">
        <section>
            ...
            <template v-for="item in items">
                <a  :href="`/admin/device/${item.id}`" class="relative mb-4 block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                    ...
                    <div class="flex flex-col justify-between p-4 leading-normal flex-wrap">
                        ...
                        <p class="font-normal text-gray-700 dark:text-gray-400">{{item.location}}</p>
                    </div>
                    <Link method="DELETE" :href="`/admin/device/${item.id}`" class="absolute right-10 top-14">
